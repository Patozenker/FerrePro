export const initVentas = [
  {
    id: 1,
    fecha: "2026-08-20",
    hora: "09:15",
    clienteId: 1,
    items: [
      { prodId: 1, qty: 1, precio: 72000, descuento: 10 },
      { prodId: 7, qty: 3, precio: 20000, descuento: 10 },
      { prodId: 9, qty: 5, precio: 5760, descuento: 10 }
    ],
    total: 139320,
    tipo: "venta",
    estado: "completada",
    formaPago: "cuenta_corriente",
    nota: "Entrega en obra Olivos"
  },
  {
    id: 2,
    fecha: "2026-08-21",
    hora: "11:40",
    clienteId: 2,
    items: [
      { prodId: 15, qty: 1, precio: 57000, descuento: 5 },
      { prodId: 16, qty: 1, precio: 57000, descuento: 5 },
      { prodId: 17, qty: 4, precio: 13260, descuento: 5 },
      { prodId: 19, qty: 6, precio: 2200, descuento: 0 }
    ],
    total: 171888,
    tipo: "venta",
    estado: "completada",
    formaPago: "efectivo",
    nota: "Descuento instalador electricista"
  },
  {
    id: 3,
    fecha: "2026-08-22",
    hora: "14:20",
    clienteId: 0,
    items: [
      { prodId: 4, qty: 1, precio: 15200, descuento: 0 },
      { prodId: 5, qty: 1, precio: 7650, descuento: 0 },
      { prodId: 25, qty: 2, precio: 8640, descuento: 0 }
    ],
    total: 40130,
    tipo: "venta",
    estado: "completada",
    formaPago: "debito",
    nota: ""
  },
  {
    id: 4,
    fecha: "2026-08-23",
    hora: "16:05",
    clienteId: 3,
    items: [
      { prodId: 11, qty: 2, precio: 81000, descuento: 0 },
      { prodId: 12, qty: 2, precio: 39200, descuento: 0 },
      { prodId: 13, qty: 3, precio: 5220, descuento: 0 },
      { prodId: 14, qty: 2, precio: 8640, descuento: 0 }
    ],
    total: 273340,
    tipo: "venta",
    estado: "completada",
    formaPago: "transferencia",
    nota: "Pintura para remodelación de oficinas"
  },
  {
    id: 5,
    fecha: "2026-08-24",
    hora: "10:50",
    clienteId: 4,
    items: [
      { prodId: 23, qty: 1, precio: 9360, descuento: 0 },
      { prodId: 22, qty: 2, precio: 1800, descuento: 0 },
      { prodId: 21, qty: 1, precio: 14280, descuento: 0 }
    ],
    total: 27240,
    tipo: "venta",
    estado: "completada",
    formaPago: "cuenta_corriente",
    nota: "Reparación baño"
  },
  {
    id: 6,
    fecha: "2026-08-25",
    hora: "17:30",
    clienteId: 5,
    items: [
      { prodId: 20, qty: 6, precio: 11560, descuento: 8 },
      { prodId: 21, qty: 3, precio: 14280, descuento: 8 },
      { prodId: 22, qty: 5, precio: 1800, descuento: 8 }
    ],
    total: 111504,
    tipo: "venta",
    estado: "completada",
    formaPago: "transferencia",
    nota: "Instalación sanitaria nueva"
  },
  {
    id: 7,
    fecha: "2026-08-26",
    hora: "11:15",
    clienteId: 0,
    items: [
      { prodId: 2, qty: 1, precio: 63000, descuento: 0 },
      { prodId: 6, qty: 1, precio: 13940, descuento: 0 }
    ],
    total: 76940,
    tipo: "venta",
    estado: "completada",
    formaPago: "tarjeta",
    nota: ""
  }
]
